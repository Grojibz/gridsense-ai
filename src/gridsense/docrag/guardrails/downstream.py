...
def invoke_structured(
    ...
    *,
    ...
    Returns ``(parsed, attempts)``, with ``parsed`` set to ``None`` when every attempt
    failed — the caller is expected to fall back deterministically rather than raise.
    """
    ...
    for attempt in range(1, retries + 2):
        try:
            ...
            # Any failure to produce a valid object is the same failure to us: a malformed
